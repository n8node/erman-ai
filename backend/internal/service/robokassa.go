...
// BuildRobokassaPaymentSignature calculates SignatureValue for a payment link.
// When resultURL2 is non-empty, Receipt and StepByStep are omitted per Robokassa docs.
...
// EncodeRobokassaResultURL2 returns the UrlEncode-encoded ResultUrl2 value for signatures.
func EncodeRobokassaResultURL2(rawURL string) string {
	return url.QueryEscape(strings.TrimSpace(rawURL))
}
...
// resetRobokassaJWSCertCache is test-only.
func resetRobokassaJWSCertCache() {
	robokassaJWSCertOnce = sync.Once{}
	robokassaJWSCert = nil
	robokassaJWSCertErr = nil
}
