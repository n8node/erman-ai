...
func New(cfg *config.Config, db *repository.Postgres, logger *slog.Logger) *Server {
	...
	userTokenBalanceRepo := repository.NewUserTokenBalanceRepository(db.Pool)
	...
	checkoutSvc := service.NewCheckoutService(planCheckoutRepo, tokenPackageCheckoutRepo, userRepo, planRepo, paymentSettingsSvc, telegramSvc, cfg)
	tokenPackageSvc := service.NewTokenPackageService(tokenPackageRepo, tokenPackageCheckoutRepo, userTokenBalanceRepo, userRepo, paymentSettingsSvc)
	tokenPackageSvc.SetCheckoutService(checkoutSvc)
	...
	consultationSvc := service.NewConsultationService(consultationRepo, userRepo, paymentSettingsSvc, telegramSvc, cfg)
	...
	paymentWebhookHandler := handler.NewPaymentWebhookHandler(paymentSettingsSvc, checkoutSvc, tokenPackageSvc, consultationSvc, logger)
	...
	billingHandler := handler.NewBillingHandler(billingSvc, checkoutSvc, tokenPackageSvc, planRepo, runRepo)
	tokenPackageHandler := handler.NewTokenPackageHandler(tokenPackageSvc)
	...
	r.Route("/api/v1", func(api chi.Router) {
		...
		api.Get("/public/billing/token-packages", tokenPackageHandler.ListPublic)
		api.Get("/public/projects", externalProjectHandler.ListPublic)
		api.Get("/public/yandex-metrika", yandexMetrikaHandler.GetPublic)
		api.Get("/public/consultations/services", consultationHandler.ListPublicServices)
		api.Get("/public/consultations/slots", consultationHandler.ListSlots)
		api.With(inquiryRL.Middleware).Post("/public/consultations/bookings", consultationHandler.CreatePublicBooking)
		api.Post("/public/consultations/checkout", consultationHandler.CreateCheckout)
		...
		api.Group(func(protected chi.Router) {
			...
			protected.Get("/billing/token-packages", tokenPackageHandler.ListPublic)
			protected.Post("/billing/switch", billingHandler.SwitchPlan)
			protected.Post("/billing/checkout", billingHandler.CreateCheckout)
			protected.Post("/billing/token-checkout", tokenPackageHandler.CreateCheckout)
		...
		api.Route("/admin", func(admin chi.Router) {
			...
			admin.Get("/consultations/services", consultationHandler.ListAdminServices)
			admin.Post("/consultations/services", consultationHandler.CreateAdminService)
			admin.Put("/consultations/services/{id}", consultationHandler.UpdateAdminService)
			admin.Get("/consultations/services/{id}/availability", consultationHandler.GetAdminAvailability)
			admin.Put("/consultations/services/{id}/availability", consultationHandler.PutAdminAvailability)
			admin.Get("/consultations/bookings", consultationHandler.ListAdminBookings)
			admin.Patch("/consultations/bookings/{id}", consultationHandler.UpdateAdminBookingStatus)
			...
			admin.Get("/token-packages", tokenPackageHandler.ListAdmin)
			admin.Post("/token-packages", tokenPackageHandler.CreateAdmin)
			admin.Put("/token-packages/{id}", tokenPackageHandler.UpdateAdmin)
