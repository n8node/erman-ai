...
func New(cfg *config.Config, db *repository.Postgres, logger *slog.Logger) *Server {
	...
	paymentWebhookHandler := handler.NewPaymentWebhookHandler(paymentSettingsSvc, checkoutSvc, consultationSvc, logger)
	...
	r.Route("/api/v1", func(api chi.Router) {
		...
		api.Group(func(protected chi.Router) {
			...
			protected.Route("/tools/geological-journal", func(journal chi.Router) {
				...
				journal.Post("/documents/{document_id}/pages/{page_id}/local-analyze", geologicalJournalDocumentHandler.AnalyzeSelectedPageLocally)
