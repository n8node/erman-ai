...
import {
  ...
  fetchAdminAudioTranscriptionSettings,
  updateAdminAudioTranscriptionAccess,
  updateAdminAudioTranscriptionSettings,
  type AudioTranscriptionAccessUser,
...
const DEFAULT_SETTINGS: AudioTranscriptionSettings = {
  model: "general",
  language_code: "ru-RU",
...
export function AdminAudioTranscriptionEditor() {
  const t = useTranslations("admin.audioTranscription");
  const [tab, setTab] = useState<Tab>("settings");
  const [settings, setSettings] = useState<AudioTranscriptionSettings>(DEFAULT_SETTINGS);
  const [users, setUsers] = useState<AudioTranscriptionAccessUser[]>([]);
  ...
  async function saveSettings() {
    ...
    setSuccess("");
    try {
      const res = await updateAdminAudioTranscriptionSettings(settings);
  ...
  async function toggleAccess(user: AudioTranscriptionAccessUser) {
    if (user.role === "superadmin") return;
    setBusyId(user.id);
    setError("");
    try {
      const enabled = !user.has_access;
      await updateAdminAudioTranscriptionAccess(user.id, enabled);
      setUsers((prev) =>
