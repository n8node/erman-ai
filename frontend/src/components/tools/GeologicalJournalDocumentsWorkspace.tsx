  ...
  async function processLocally() {
    ...
    try {
      for (const pageNumber of selectedPages) {
        const page = active.pages.find((item) => item.page_number === pageNumber);
        if (page) await analyzeGeologicalJournalDocumentPageLocally(active.document.id, page.id);
      }
      await loadActive(active.document.id);
    } catch (err) { setError(err instanceof Error ? err.message : "Не удалось выполнить локальный OCR выбранных страниц"); }
    finally { setBusy(false); }
  }
