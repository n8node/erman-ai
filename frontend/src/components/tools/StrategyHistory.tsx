...
export function StrategyHistory() {
  const t = useTranslations("strategy.history");
  ...
  useEffect(() => {
    listRuns({ tool_slug: "strategy", limit: 50 })
    ...
    <div className="space-y-6">
      ...
      {items.length === 0 ? (
        <div className="rounded-xl border border-border bg-bg p-8 text-center">
          ...
          <Link href="/tools/strategy" className="mt-4 inline-block text-sm text-accent hover:underline">
            {t("emptyAction")}
          </Link>
        </div>
      ) : (
              ...
              {items.map((row) => (
                <tr key={row.id} className="border-b border-border last:border-0">
                  ...
                  <td className="px-4 py-3 space-x-2">
                    <button
                      type="button"
                      onClick={() => router.push(`/tools/strategy?run=${row.id}`)}
